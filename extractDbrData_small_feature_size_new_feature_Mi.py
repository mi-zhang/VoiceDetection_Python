...
def handleDbrFile(filename, filedir):    
    ...
    # for audio data
        ...
        if row[1] == 0:  # write the audio data
            ...
            NTP_FILE.write('\n')
        elif row[1] == 31:  # writes audio features (NEW)
            """
            MyDataTypeConverter.toByta(voicingFeatures, inferanceResults_one, observationProbability,
            numberOfPeaks, autoCorrelationPeaks, autoCorrelationPeakLags), audioFromQueueData.sync_id);

            public static byte[] toByta(float[] voicingFeatures, byte[] inferanceResults,
            float[] observationProbability, int[] numberOfPeaks, float[] autoCorrelationPeaks,
            short[] autoCorrelationPeakLags)

           """
            data = row[2]
            energy_ratio = to_Float(data[0:4])
            ...
            inference_result = ord(data[24])
            print "feature values: ", energy_ratio, maxAcorrPeakVal, maxAcorrPeakLag, spectral_entropy, rel_spectral_entropy, energy
            print "inference results: ", inference_result
            emissionProbabilitiesUnvoiced = to_Float(data[25:29])
            emissionProbabilitiesVoiced = to_Float(data[29:33])
            print "inference probabilities: ", emissionProbabilitiesUnvoiced, emissionProbabilitiesVoiced
            numAcorrPeaks = to_Int(data[33:37])
            print "numAcorrPeaks: ", numAcorrPeaks
            acorrPeakValueArray = []
            index = 37
...
if __name__ == "__main__":
    ...
    handleDbrFile('1396426889_351746051964131.dbr', 'C:\Users\Mi Zhang\Desktop\data_new_feature')
